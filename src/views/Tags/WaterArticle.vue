<template>
    <div class="container">
        <base-button mode="reverse" v-if="onlyWaterTag.length" @click="router.back()" class="my-4">Back</base-button>
        <div class="row">
            <div class="text-center my-5 py-5" v-if="!onlyWaterTag.length">
                <p>No articles to be found here</p>
                <base-button @click="router.push('/')">Home Page</base-button>
            </div>
            <div class="pb-4" v-if="onlyWaterTag.length">
                <h3>Water Articles</h3>
                <hr class="m-0">
            </div>

            <div v-if="onlyWaterTag.length" class="col-sm-3 d-block mb-4" v-for="item in onlyWaterTag" :key="item">
                <div class="card grid">
...
<script setup>
 ...
 import { useRouter } from 'vue-router';
    ...
    const onlyWaterTag = computed(() => {
        return filteredTagItem.value = articlesStore.allArticles.filter(itmm => itmm.tag.includes('water'))
    })
</script>
