<template>
    <div class="container">
        <base-button mode="reverse" v-if="onlySkyTag.length" @click="router.back()" class="my-4">Back</base-button>
        <div class="row">
            <div class="text-center py-5 my-5" v-if="!onlySkyTag.length">
                <p>No articles to be found here</p>
                <base-button @click="router.push('/')">Home Page</base-button>
            </div>
            <div class="pb-4" v-if="onlySkyTag.length">
                <h3>Sky Articles</h3>
                <hr class="m-0">
            </div>
            <div v-if="onlySkyTag.length" class="col-sm-3 d-block mb-4" v-for="item in onlySkyTag" :key="item">
                <div class="card grid">
...
<script setup>
 ...
 import { useRoute, useRouter } from 'vue-router';
    ...
    const onlySkyTag = computed(() => {
        return filteredTagItem.value = articlesStore.allArticles.filter(itmm => itmm.tag.includes('sky'))
    })
</script>
