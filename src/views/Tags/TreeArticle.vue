<template>
    <div class="container">
        <base-button mode="reverse" v-if="onlyTreeTag.length" @click="router.back()" class="my-4">Back</base-button>
        <div class="row">
            <div class="text-center py-5 my-5" v-if="!onlyTreeTag.length">
                <p>No articles to be found here</p>
                <base-button @click="router.push('/')">Home Page</base-button>
            </div>
            <div class="pb-4" v-if="onlyTreeTag.length">
                <h3>Tree Articles</h3>
                <hr class="m-0">
            </div>
            <div v-if="onlyTreeTag.length" class="col-sm-3 d-block mb-4" v-for="item in onlyTreeTag" :key="item">
                <div class="card grid">
...
<script setup>
 ...
 import { useRoute, useRouter } from 'vue-router';
    ...
    const onlyTreeTag = computed(() => {
        return filteredTagItem.value = articlesStore.allArticles.filter(itmm => itmm.tag.includes('tree'))
    })
</script>
